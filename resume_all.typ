...
#let NEW_SECTION_SPACE = -0.6em
#let NEW_ITEM_SPACE = 0.7em
...
#text(size: SizeFont.expr)[
  ...
  software development and problem-solving. I am a dedicated lifelong
  learner with lots of curiosity! In my free time you can find me learning
  languages such as Spanish or practicing solving Rubiks cubes. I am very
  interested in music and cooking too!
]
...
#exprItem(
  ...
  [
    ...
    Mines EE Classes: *Circuits*, *Digital Logic*, *PCB Design*, and
...
#table(
  ...
  columns: 2,
  row-gutter: -0.5em,
  [*Languages*], [| Python3, C/C++, Rust, Go, JavaScript, Postgres SQL, Verilog],
  [*Dev. Tools*], [| Linux, Docker, Git, Makefile, Nix, Nginx, S3 Bucket],
  [*Libraries*], [| Express.js, Flask, NumPy, Pandas, Matplotlib, SciKitLearn],
  [*Software Tools*], [| PlatformIO, Fusion360, EagleCAD, FreeCAD, KiCAD, Quartus],
...
#exprItem(
  ...
  list(
    [
      Managed a team of 7 intern engineers whilst working closely with other
      advisors.
    ],
    [
      Advised on *Python*, *Flask*, *Hardware* configuration, and industry
      programming practices.
    ],
    [
      Developed and open-sourced a *Python* tool for importing *Jupyter
      Notebooks*.
    ],
  ),
)
...
#exprItem(
  ...
  box[
    #text[
      Self hosted music streaming service with future plans of developing
      compatible hardware.
    ]
    #list(
      [
        Hand rolled, multiprocess safe, authentication using *python*, *JWTs*,
        and *sockets*.
      ],
      [
        Safe data ingest with *mimetype detection*, conversion to *Vorbis ogg*,
        and storage in *postgres*.
      ],
      [
        Web UI written in modular *JavaScript*, using *Flask* templates on
        static routes.
      ],
      [
...
#exprItem(
  ...
  box[
    #text[
      NovaMill is a work in progress Open Hardware desktop PCB Mill with the
      goal of making rapid PCB prototyping easier as faster.
    ]
    #list(
      [
      ...
      [
        *Pi Zero 2 W* running custom *Linux* distribution for on-device
...
#exprItem(
  ...
  box[
    #text[
      Working towards implementing my custom *8-bit* Computer based on *74HC*
      chips. The architecture is based on my *Distributed Custom ISA Execution
      Environment* software project.
    ]
    #list(
      [
        Designed and iterating on *custom toolchain* to edit my CPU's
        *microcode*.
      ],
      [
        Implementing *FPGA* adaptaion to be implemented in the
...
#exprItem(
  "Distributed Custom ISA Execution Environment (BL-8 Computer Architecture)",
  "Author",
  monthYear(4, 2025),
  TODAY,
  box[
    #text[
      Designed a custom computing ecosystem, including an *Instruction Set
      Architecture* (ISA), *Assembler*, *Virtual Machine* (VM), and a *TCP*
      machine state transmission networking protocol. The system enables
      distributed execution of custom machine code across a
      client-router-server architecture.
    ]
    #list(
      [
        The custom *8-bit ISA* is executed by a corresponding *VM* developed in
        *Go*, featuring 256-bytes of RAM.
      ],
      [
        The custom *TCP* networking protocol implemented in *Go*, enables
        distributed execution by transmitting machine state packets between
        client, router, and server.
      ],
      [
        The *assembler* translates human-readable assembly into the custom ISA's
        machine code using a *LL(1) Tabular Parser* dynamically generated
        applying *Formal Language Theory*.
      ],
    )
  ],
)
